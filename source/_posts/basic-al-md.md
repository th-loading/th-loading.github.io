...
1. 容易误判的性质  
    ...
    ... lc-303r, 基于第一条错误的式子，对于bit类的f函数，范围在10^9, 而异或基于bit，无法通过sort获取可以有序DP的路径，无法通过枚举每个1来达到化简的目的(集合定义的简化) Stuck - 第一条定义式就已经错误了  
    2. 存在多层逻辑依赖关系的问题  
    ...
    3. 构造最优策略类问题  
    !!! 判断一个选择造成的影响，一种不经证明的贪心，似乎总有一定的机会，却未触及核心的观察，判断一条路径的可行性（对需要解决问题难度的预估) 尤为重要  
    ? 不断枚举，构造一步、两步的贪心，简化了部分的转移条件，无法构造出一种解法 - 考虑忽略了一些重要sk优化，始终没有接触到真正的问题，循环论证的观察  
...
3. 计算类
   快速乘/幂 乘法 - 指数  
   异或的性质 不具备分配律 除了mask等方法，难以直接通过有序的DP转移，可以通过最后一位的规律，进行基本的构造，结合重复消去的特点，能够缩小一些范围, secret x,  
   取整函数，通过乘积形式，拆括号  
